...
namespace ProcessorTest2
{
	...
	public:
		...
		{
			return QFileInfo(QString("%1/NetworkOverallTest/myprocessor2_testfile%2").arg(
				QStandardPaths::writableLocation(QStandardPaths::TempLocation), QString::number(index)));
		}
		static bool createTestFile(const int& index)
		{
			QDir().mkpath(
				QString("%1/NetworkOverallTest").arg(QStandardPaths::writableLocation(QStandardPaths::TempLocation)));
